...
# get Database data
db_password = ""
if node.roles.include? "manila-controller"
  db_password = node[:manila][:db][:password]
else
  # pickup password to database from manila-controller node
  node_controllers = search(:node, "roles:manila-controller") || []
  if node_controllers.length > 0
    db_password = node_controllers[0][:manila][:db][:password]
  end
end
